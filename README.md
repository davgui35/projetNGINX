# projetNGINX
# Simple authentification en php
...
```
_Une fois installé, changez ces valeurs :_
```bash
...
Enfin, activons notre hôte et redémarrons Nginx :
 ...
 sudo service nginx restart
``` 
A ce stade, nous avons un serveur fonctionnel utilisant HTTP. Avant de l’utiliser en production, nous devons augmenter la sécurité de celui ci et de notre système.
...
On peut choisir le port (ici 8080) pour garder un vhost spécifique.
...
### Générate token for jenkin pour s'authentifier
...
### Ajouter sonar-scanner dans jenkins
_sonar-scanner cli_
```bash
...
### Change sonar-scanner.properties
Properties:
...
### Creer un projet dans sonarqube
-> generate un token
...
start sonarqube -> 
```bash
sudo su -c '/opt/sonarqube/bin/linux-x86-64/sonar.sh start' sonar
```
